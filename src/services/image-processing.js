export const keepRatio = ({ width, height }) => size => ((width <= size || height <= size) && { resizeWidth: width, resizeHeight: height })
  || (width > height && { resizeWidth: Math.round((size * width) / height), resizeHeight: size }) || {
  resizeWidth: size,
...
export const cropSquare = ({ width, height }) => {
  ...
  return [Math.round((width - minSize) / 2), Math.round((height - minSize) / 2), minSize, minSize]
}
...
export const getImgSrc = img => {
  ...
  return ctx.canvas.toDataURL(img, 'image/png', 1)
}
